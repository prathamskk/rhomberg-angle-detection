...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    while cap.isOpened():
        ...
        try:
            ...
            head = [(lefteye[0]+righteye[0])/2 ,  (lefteye[1]+righteye[1])/2]
            centerleg = [(leftheel[0]+rightheel[0])/2 ,  (leftheel[1]+rightheel[1])/2]
            reference = [0.5, 0.5]
            # print(head)
            ...
            # Calculate angle
            angle = calculate_angle(head, centerleg, reference)
            print(angle)
            # Visualize angle
            cv2.putText(image, str(angle), 
                           tuple(np.multiply(elbow, [320, 240]).astype(int)), 
